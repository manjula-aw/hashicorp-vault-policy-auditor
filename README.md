...
1. **Select Input:** Click "Browse Folder" to select the directory containing your Policy files.
2. **Filter (Optional):** Enter extensions in the box (e.g., `.hcl, .txt`) to filter specific files. Leave blank to scan files with **no extension** (default).
...
**Default Mode:** Scans only files with **NO extension** (standard Vault storage format).
...
**Scan Specific Extensions:** Use `--ext` to specify allowed extensions (comma-separated).

```cmd
python vault_audit_cli.py my_policies_folder --ext .hcl,.txt
...
```cmd
python vault_audit_cli.py policies --ext .hcl --html report.html --excel report.xlsx
...
```bash
python3 vault_audit_cli.py policies --ext .hcl --html report.html --excel report.xlsx
...
```cmd
python vault_audit_cli.py policies --ext .hcl --fail-on-critical
...
**2. Run the Audit**
Execute the CLI tool against this folder. **Note:** Since these are `.hcl` files, you must use `--ext .hcl`.

**Windows:**

```cmd
python vault_audit_cli.py test_policies --ext .hcl --html verification_report.html
...
```bash
python3 vault_audit_cli.py test_policies --ext .hcl --html verification_report.html
