...
class VaultAuditEngine:
    ...
    def export_html(self, file_path):
        ...
        <style>
            :root{{--bg:#f4f6f8;--white:#fff;--danger:#e74c3c;--warning:#f39c12;--success:#27ae60;--primary:#2c3e50;}} 
        ...
        <div class="header"><div><h1>Vault Policy Audit Report</h1><div style="color:#777">{timestamp}</div></div><div><span class="badge bg-ok">v22.0</span></div></div>
        ...
        <div class="card"><h2>1. Risk Visualization</h2><div class="mermaid">{graph_def}</div></div>
        
        <div class="card"><h2>2. Security Risks</h2><table><thead><tr><th>Severity</th><th>Policy</th><th>Path</th><th>Issue</th><th>Fix</th></tr></thead><tbody>"""
        ...
        for i in self.audit_issues:
            html_content += f"<tr><td><span class='badge bg-{i['sev'].lower()}'>{i['sev']}</span></td><td>{html.escape(i['pol'])}</td><td><span class='path-mono'>{html.escape(i['path'])}</span></td><td>{html.escape(i['msg'])}</td><td>{html.escape(i['fix'])}</td></tr>"
        html_content += "</tbody></table></div>"

        html_content += """<div class="card" style="margin-top:20px;"><h2>3. Access Matrix</h2><table><thead><tr><th>Path</th><th>Accessible By (Policy)</th><th>Capabilities</th></tr></thead><tbody>"""
        for path in sorted(self.path_matrix.keys()):
            first = True
            for e in self.path_matrix[path]:
                ...
                via_txt = f"<br><small style='color:gray'>via {html.escape(e['via'])}</small>" if e['via'] else ""
                html_content += f"<tr>{p_cell}<td><b>{html.escape(e['policy'])}</b>{via_txt}</td><td>{', '.join(e['caps']).upper()}</td></tr>"
                first = False
        html_content += "</tbody></table></div>"

        html_content += """<div class="card" style="margin-top:20px;"><h2>4. Policy Inspector</h2><table><thead><tr><th>Policy</th><th>Path</th><th>Matches</th></tr></thead><tbody>"""
            ...
            for pb in data['parsed'].get('path', []):
                for p_str, r in pb.items():
                    ...
                    if "*" in p_str or "+" in p_str:
                        ...
                        if m: m_html = "<br><small style='color:gray'>↳ " + ", ".join(m) + "</small>"
        ...
        html_content += """</tbody></table></div><div class="card" style="margin-top:20px;"><h2>5. Processing Log</h2><table><thead><tr><th>File</th><th>Status</th><th>Details</th></tr></thead><tbody>"""
